...
class AloeModule(LightningModule):
    ...
    def step(self, batch: Any, optimizer_idx: Any):
        if optimizer_idx==0:
            visuals, question, target = batch["descriptive"]
        else:
            visuals, question, target = batch["counterfactual"]
            
        with torch.no_grad():
            text_embd = self.bert(question["ids"], question["mask"]).last_hidden_state
        logits = self.forward(text_embd=text_embd, visuals_emdb=visuals, optimizer_idx=optimizer_idx)
    ...
    def training_step(self, batch: Any, batch_idx: int, optimizer_idx: Any):
        loss, preds, targets = self.step(batch, optimizer_idx=optimizer_idx)

        # log train metrics
        if optimizer_idx==0:
            acc_des = self.train_acc_des(preds, targets)
            self.log("train/acc_des", acc_des, on_step=False, on_epoch=True, prog_bar=True)    
        else:
            acc_cnt = self.train_acc_cnt(preds, targets)
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        loss1, preds1, targets1 = self.step(batch, optimizer_idx=0)
        loss2, preds2, targets2 = self.step(batch, optimizer_idx=1)

        # log val metrics
        acc_des = self.val_acc_des(preds1, targets1)
        acc_cnt = self.val_acc_cnt(preds2, targets2)
        ...
        return {"loss": loss1+loss2, "preds": (preds1, preds2), "targets": (targets1, targets2)}
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        # log test metrics
        acc_des = self.test_acc_des(preds1, targets1)
        acc_cnt = self.test_acc_cnt(preds2, targets2)
    ...
    def configure_optimizers(self):
        ...
        return [opt, opt], [{"scheduler": scheduler, "interval": "step"}]
